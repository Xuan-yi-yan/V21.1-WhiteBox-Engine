...
p3l = P3L_AttributeLinkage(attr_dim=128, num_attr_values=300).to(DEVICE)
p7_input_proj = nn.Linear(384, 256, bias=False).to(DEVICE)  # A256D+P3(128D)→256D
...
class ABC_StageA(nn.Module):
    """A: 结构决策 15类 — 对齐14模块P3颗粒度"""
    def __init__(self, in_dim=384, hidden=128, n_classes=15):
        super().__init__()
...
class ABC_StageB(nn.Module):
    """B: 内容填充 128D — 对齐P3"""
    def __init__(self, hidden=128, content_dim=128):
        super().__init__()
        self.fuse = nn.Sequential(nn.Linear(128+128+128, 256), nn.GELU(), nn.Linear(256, 128), nn.GELU(), nn.Linear(128, content_dim))
        for m in self.modules():
            if isinstance(m, nn.Linear): nn.init.xavier_uniform_(m.weight, gain=0.5); nn.init.zeros_(m.bias)
...
class ABC_StageC(nn.Module):
    """C: 语气精炼 48D"""
    def __init__(self, content_dim=128, hidden=96, out_dim=48):
        super().__init__()
...
assert p7_input_proj.in_features == 384, f"p7_in in={p7_input_proj.in_features}"
...
def words_to_attr_vec(words):
    ...
    if args.turbo or len(attr_cache) > 100:  # 有预缓存就用
        ...
        for w in words[:80]:
            ...
            else: vecs.append(torch.zeros(128, device=DEVICE))
        if len(vecs) < 80:
            pad = torch.zeros(80-len(vecs), 128, device=DEVICE)
...
for ep in range(1, args.epochs+1):
    ...
    while remaining:
        ...
        attr_for_p7 = torch.zeros(bs, max_len_a, 128, device=DEVICE)
        for i, (ids_a, ids_b, A_words, _) in enumerate(batch):
...
with torch.no_grad():
    ...
    wo_pool = torch.zeros(1, 128, device=DEVICE)
    wo_exp = wo_pool.unsqueeze(1).expand(1, attr_vec.shape[1], 128)
